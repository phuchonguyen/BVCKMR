...
Z     = matrix(mvrnorm(n = n, mu=c(rep(0,M)), 
                       Sigma = matrix(data = c(1,0.2,0.3,0.45,0.15, 
                                               0.2,1,0.4,0.45,0.3, 
                                               0.3,0.4,1,0.3,0.15, 
                                               0.45,0.45,0.3,1,0.2, 
                                               0.15,0.3,0.15,0.2,1), nrow=M)), 
               byrow=FALSE, nrow=n)
...
X				= cbind( 
  ...
  matrix( rep(scale(matrix(sample(1:2, n, replace = TRUE))), each=T) )
  )
beta		= rep(1,p)
...
Y = matrix(rnorm(n=N, mean=0, sd=res.sd)) + 
  X %*% beta + 
  rep(0.5*(Z[,1]^2 + Z[,2]^2 - Z[,4]^2 - Z[,5]^2 + 0.5*Z[,4]*Z[,5] + Z[,4] + Z[,5]), each=T) + 
  matrix(rep(0.5*(Z[,1]^2 - Z[,2]^2 - Z[,1]*Z[,2] + Z[,3]^2 + Z[,4] - Z[,5]), each=T)) * rep(age, n) + 
  U %*% b
